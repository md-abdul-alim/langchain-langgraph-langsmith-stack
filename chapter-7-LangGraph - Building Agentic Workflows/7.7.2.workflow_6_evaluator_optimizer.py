...
def generate_description(state: OptimizationState) -> OptimizationState:
    ...
    if iteration == 1:
        ...
        Requirements:
        - Headline: Catche and concise (max 10 words)
    ...
    formatted_description = f"""
        ...
        KEY BENEFITS: {chr(10).join([f"* {benefit}" for benefit in description.key_benefits])}

        CALL TO ACTION: {description.call_to_action}
    """

    print("Generation Description: ")
    print('-'*70)
    print(formatted_description + "n")
...
def evaluate_description(state: OptimizationState) -> OptimizationState:
    ...
    - Specific strenghts (what's working well)
    ...
    print(f"OClarity {evaluation.clarity_score}/10")
...
def should_continue(state: OptimizationState) -> Literal["optimizer", "end"]:
    ...
    if state['iteration_count'] > state['max_iterations']:
        print(f"\n MAX ITERATIONS RECHED: Stopping at iteration {state['iteration_count'] - 1}")

        return "end"
    
    print(f"\n CONTINUING: Routing beck to optimizer for iteration {state['iteration_count']}")
