...
def classify_query(state: SupportState) -> SupportState:
    ...
    classfier_llm = llm.with_structured_output(QueryClassification)
    ...
    prompt=f"""
        classify the following customer support query into one of these categories:
            ...
            - general: General questions, feature inquiries, information requrests
        
        Customer Query: {state['customer_query']}
        Classify accurately based on the primary intent.
    """

    classification = classfier_llm.invoke(prompt)
...
def handle_billing(state: SupportState) -> SupportState:
    print(f"BILLING  HANDLER: Processing billing query")
    prompt = f"""
...
def handle_technical(state: SupportState) -> SupportState:
    print(f"TECHNICAL  HANDLER: Processing technical query")
    prompt = f"""
        You are a technical specialist. Handle this customer query:
        {state['customer_query']}
...
def handle_refund(state: SupportState) -> SupportState:
    print(f"REFUND  HANDLER: Processing refund query")
    prompt = f"""
...
def handle_general(state: SupportState) -> SupportState:
    ...
    print(f"GENERAL HANDLER: Processing general query")
    prompt = f"""
...
builder.add_node("refund", handle_general)
...
# Example Prompts
billing_prompt = "I was charged twice for my subsciption this month. Can you help?"
technical_prompt = "The app keeps crashing when I try to upload files larger that 10M"
